...
const formSchema = z.discriminatedUnion("catalogType", [
    ...
    z.object({ catalogType: z.literal("estadosAcreditacionDoc"), nombre: z.string().min(2, "Mínimo 2 caracteres"), orden: z.coerce.number().min(1, "Debe ser > 0") }),
]);
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 py-4">
        ...
        {(catalogType === 'ambitos' || catalogType === 'caracteristicas' || catalogType === 'elementosMedibles' || catalogType === 'tiposDocumento' || catalogType === 'estadosAcreditacionDoc') && <FormField control={form.control} name="orden" render={({ field }) => (<FormItem><FormLabel>Orden</FormLabel><FormControl><Input type="number" {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>)} />}
